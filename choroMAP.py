...
flights = pd.read_csv('./CLONED/Data/flights.csv')
flights = flights.groupby('year').sum().reset_index()

olympics = pd.read_csv('./CLONED/Data/2018WinterOlympics.csv')

#### NESTED BAR PLOT TRACES ### 
df = pd.read_csv('./CLONED/Data/2018WinterOlympics.csv')
trace1 = go.Bar(
        x=df['NOC'], 
        y=df['Gold'],
        name = 'Gold',
...
trace2 = go.Bar(
        ...
        y=df['Silver'],
        name='Silver',
        #marker=dict(color='#1919FF') # set the marker color to silver
        )
trace3 = go.Bar(
        x=df['NOC'],
        y=df['Bronze'],
        name='Bronze',
        #marker=dict(color='#ffa500') # set the marker color to bronze
        )
...
DEFAULT_COLORSCALE = ["#42005c", "#6c0051", "#af006a", "#e00062", "#ea3575","#42005c", "#6c0051", "#af006a", "#e00062", "#ea3575", "#42005c", "#6c0051", "#af006a", "#e00062", "#ea3575", "#d9f271"]
...
app.layout = html.Div(children=[
	...
	html.Div([
		html.Div([
			html.Div([
				html.H4(children='idk what this data is whtever'),
				html.P('change yr by sliding:'),
			]),
		...
		html.P('Heatmap yr {0}'.format(min(YEARS)),
			id = 'heatmap-title',
		...
		html.Div([
			html.P('laksdjflajs lkjdinformations lkjasdlkfjaslkj dflaksjd fljasdlkfjasldjflkasjdf .'
			)
		], style={'margin':20})
	...
	html.Div([
		...
		dcc.Dropdown(
		    options=[{'label': 'Histogram of total number (single year)', 'value': 'show_absolute_deaths_single_year'},
					{'label': 'Histogram of total (all yeras)', 'value': 'absolute_deaths_all_time'},
					{'label': 'Age-adjusted death rate (single year)', 'value': 'show_death_rate_single_year'},
					{'label': 'somethigin else ', 'value': 'death_rate_all_time'}],
        ...
        html.Div([
            dcc.Graph(
                ...
                figure = {
                    ...
                    'layout': go.Layout(
                        title = 'Blah blah fund allocation laksjdflajsdf', 
                        )
                    }
                )
    ...
    ######## BAR 1 row 2 cols############
        html.Div([
            html.Div([
                dcc.Graph( 
                    id='olympicsCountriesMedals',
                    figure={
                        'data': [

                            go.Bar(x=olympics['Total'], 
                                y=olympics['NOC'],
                                orientation='h')
            ...
            html.Div(
                [
                    dcc.Graph(
                        id='passengers_per_year',
                        figure={
                            'data': [
                                go.Bar(x=flights['passengers'], 
                                    y=flights['year'],
                                    orientation='h',
                                    marker={'color':'#ffa500'})
                                ],
                            ...
                            'layout': go.Layout(
                                ...
                                title = 'Project Top - Top6 Codes',
                                xaxis = {'title' : 'Count'},
                                yaxis = {'title' : 'Project Code'},
                                hovermode = 'closest'
...
def update_map_title(year):
	return 'poisonings heatmap in year {0}'.format(year)
